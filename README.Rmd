...
```{bash, eval=FALSE}
sudo docker run -p 8787:8787 -v リンクさるローカルパス:/home/rstudio/project \
...
## Docker側のホームディレクトリ
　Dockerのユーザ名は`rocker/*`と同じく`rstudio`です。`rstudio`ユーザのホームディレクトリ配下には以下のサブディレクトリがあります。

サブディレクトリ名 | 用途など
-------------------|-----------------------------------------------------------
project            | プロジェクト用ディレクトリ（ローカルパスのマウントポイント）
